...
hirschberg <- function(seq1, seq2, align, match, mismatch, gap){
    ...
    else  #NW algorithm
    {
        ...
        {
            align <- hirschberg(seq1[1:x_mid], DNAString(""), align, match, mismatch, gap)# call hirschberg function for the first half of seq1 and for an empty DNAString object
        }
        else
        {
            align <- hirschberg(seq1[1:x_mid], seq2[1:y_mid], align, match, mismatch, gap)# call hirschberg function for the first half of seq1 and for the first part of seq2
        }
        
        # The second half
        if ((x_mid + 1) > x_len) # seq1 cannot be further divided
        {
            align <- hirschberg(DNAString(""), seq2[(y_mid+1), y_len], align, match, mismatch, gap)# call hirschberg function for an empty DNAString object and the second half of seq2
        }
        ...
        {
            align <- hirschberg(seq1[(x_mid+1):x_len], seq2[(y_mid+1), y_len], align, match, mismatch, gap)# call hirschberg function for the second half of seq1 and the second part of seq2
        }
    }
    return(align)
}
...
final<- hirschberg(seq1, seq2, c(), match, mismatch, gap)
